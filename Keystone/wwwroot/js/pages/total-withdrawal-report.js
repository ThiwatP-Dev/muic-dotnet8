...
var workSheetName = $('.withdrawal-worksheet-name').val();
for (var x = 1; x <= 1; x++) {
    array1[x-1] = x;
    array2[x-1] = x + 'th';
}
...
var tablesToExcel = (function () {
    ...
    return function (table, name, filename) {
        ...
        for (var k = 0; k < tables.length; ++k) {
            var exceltable;
            if (!tables[k].nodeType) exceltable = document.getElementById("withdrawal-report-export");
            ctx['table' + k] = exceltable.innerHTML;

        }

        document.getElementById("withdrawal-file-name").href = uri + base64(format(allOfIt, ctx));
        document.getElementById("withdrawal-file-name").download = filename;
        document.getElementById("withdrawal-file-name").click();
    }
})();