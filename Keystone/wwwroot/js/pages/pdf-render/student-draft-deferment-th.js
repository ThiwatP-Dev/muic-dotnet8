...
var page = {
    ...
    p5 : ["เกิดวันที่", "พ.ศ.", "เป็นนักศึกษาปีที่", "ปีการศึกษา",
          "ระดับ" ,"คณะ", "สาขา",
...
function renderCertificate() {
    ...
    var birthFullTime = dataBody.BirthDate.split('-'),
        birthDate = parseInt(birthFullTime[2].substring(0,2)),
        birthMonth = parseInt(birthFullTime[1])-1;
        birthYear = parseInt(birthFullTime[0])+543;
    ...
    PdfSettings.format(report, textFormats.sansN)
    report.text(`${ page.p5[0] } ${ birthDate } ${ getMonthText(language, birthMonth) } ${ page.p5[1] } ${ birthYear }`, centerX, currentY += lineYN, PdfSettings.style(textStyles.CertificateCenter))
          .text(`${ page.p5[2] } ${ dataBody.StudyYear } ${ page.p5[3] } ${ dataBody.Year }`, centerX, currentY += lineYN, PdfSettings.style(textStyles.CertificateCenter))
          .text(`${ page.p5[4] }${ dataBody.AcademicLevelName }`, centerX, currentY += lineYN, PdfSettings.style(textStyles.CertificateCenter));

    PdfSettings.format(report, textFormats.sansBoldN)
    report.text(`${ page.p5[5] }${ dataBody.FacultyName }`, centerX, currentY += lineYN, PdfSettings.style(textStyles.CertificateCenter))
          .text(`${ page.p5[6] }${ dataBody.DepartmentName }`, centerX, currentY += lineYN, PdfSettings.style(textStyles.CertificateCenter));

    PdfSettings.format(report, textFormats.sansN)
    report.text(page.p5[7], centerX, currentY += lineYN, PdfSettings.style(textStyles.CertificateCenter))
          .text(`${ page.p5[8] } ${ getDateText(language, issuedDate) }`, centerX, currentY += lineYL, PdfSettings.style(textStyles.CertificateCenter));
