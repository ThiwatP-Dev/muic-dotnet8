...
{
    [PermissionAuthorize("Concentration", "")]
    public class ConcentrationController : BaseController
    {
        ...
        public ConcentrationController(ApplicationDbContext db,
        ...
        public IActionResult Index()
        {
            var models = _db.SpecializationGroups.IgnoreQueryFilters()
                                                 .Where(x => x.Type == SpecializationGroup.TYPE_CONCENTRATION_CODE)
                                                 .ToList();
            return View(models);
        }

        [PermissionAuthorize("Concentration", PolicyGenerator.Write)]
        ...
        public ActionResult Create(SpecializationGroup model)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    model.Type = SpecializationGroup.TYPE_CONCENTRATION_CODE;
        ...
        public IActionResult Details(long concentrationId, string returnUrl)
        {
            ViewBag.ReturnUrl = returnUrl;
            ViewBag.IsSpecialGroup = true;
            var model = _db.SpecializationGroups.IgnoreQueryFilters()
                                                .Where(x => x.Id == concentrationId
                                                            && x.Type == SpecializationGroup.TYPE_CONCENTRATION_CODE)
            ...
            model.CourseGroups = _curriculumProvider.GetCourseGroupBySpecialGroup(concentrationId);
            return View(model);
        }
