...
{
    public class QuestionnaireByInstructorController : BaseController
    {
        ...
        public QuestionnaireByInstructorController(ApplicationDbContext db,
        ...
        public ActionResult Index(Criteria criteria, int page)
        {
            ...
            if (criteria.TermId == 0)
            {
                ...
                return View(new QuestionnaireApprovalViewModel
                            {
                                Criteria = criteria
                            });
            ...
            bool IsQuestionnaireMember = _db.QuestionnaireMembers.Any(x => x.InstructorId == userInstructorId);
            if (IsQuestionnaireMember)
            {
                sectionViewModel = GetQuestionnaireByQuestionnaireMember(criteria.TermId);
            }
            // PROGRAM DIRECTOR
            else 
            {
                var facultyMember = _db.FacultyMembers.AsNoTracking()
                                                      .Where(x => x.InstructorId == userInstructorId 
                                                                  && x.Type == "pd")
                                                      .ToList();
                ...
                if (facultyMember != null && facultyMember.Any())
                {
                    ...
                    sectionViewModel = GetQuestionnaireByPD(criteria.TermId, filterCourseIds);
                }
            }

            // INSTRUCTOR
            sectionViewModel.AddRange(GetQuestionnaireByInstructor(criteria.TermId, userInstructorId));
        ...
        private List<QuestionnaireApprovalSection> GetQuestionnaireByQuestionnaireMember(long termId)
        {
            var questionnaires = _db.QuestionnaireApprovals.AsNoTracking()
                                                           ...
                                                           .Where(x => x.TermId == termId && x.IsActive)
                                                           .ToList();
            ...
            {
                var responseCount = allRelatedResponse.FirstOrDefault(x => x.SectionId == questionnaire.SectionId)?.Group.Distinct().Count() ?? questionnaire.TotalSurvey;
        ...
        private List<QuestionnaireApprovalSection> GetQuestionnaireByPD(long termId, List<long> courseIds)
        {
            var questionnaires = _db.QuestionnaireApprovals.AsNoTracking()
                                                           .IgnoreQueryFilters()
                                                           .Include(x => x.Course)
                                                           ...
                                                           .Where(x => x.TermId == termId
                                                                       && x.IsActive
                                                                       && courseIds.Contains(x.Course.Id))
                                                           .ToList();
                ...
                var questionnaireApprovalSection = new QuestionnaireApprovalSection()
                                                    {
                                                        ...
                                                        Total = questionnaire.TotalSectionScore,
                                                        TotalRelatedSection = questionnaire.TotalSectionMJScore,
                                                        IsDisabled = (questionnaire.Status == "w" ? true : false)
                                                    };
                sectionViewModel.Add(questionnaireApprovalSection);
            }
            return sectionViewModel;
        ...
        private List<QuestionnaireApprovalSection> GetQuestionnaireByInstructor(long termId, long instructorId)
        {
            var questionnaires = _db.QuestionnaireApprovals.AsNoTracking()
                                                           .Include(x => x.Course)
                                                           .Include(x => x.Section)
                ...
                var questionnaireApprovalSection = new QuestionnaireApprovalSection()
                                                    {
                                                        ...
                                                        Total = questionnaire.TotalSectionScore,
                                                        TotalRelatedSection = questionnaire.TotalSectionMJScore,
                                                        IsDisabled = (questionnaire.Status == "w" || questionnaire.Status == "s" ? true : false)
                                                    };
                sectionViewModel.Add(questionnaireApprovalSection);
            }
            return sectionViewModel;
