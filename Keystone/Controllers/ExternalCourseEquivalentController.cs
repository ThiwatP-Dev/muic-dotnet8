...
{
    [PermissionAuthorize("ExternalCourseEquivalent", "")]
    public class ExternalCourseEquivalentController : BaseController
    {
        public ExternalCourseEquivalentController(ApplicationDbContext db,
        ...
        public IActionResult Index(int page, Criteria criteria)
        {
            ...
            var model = _db.CourseEquivalents.Include(x => x.Course)
                                                 .ThenInclude(x => x.TransferUniversity)
                                             .Include(x => x.EquilaventCourse)
                                             .Where(x => x.Course.TransferUniversityId != null
                                                         && (criteria.TransferUniversityId == 0
                                                             || x.Course.TransferUniversityId == criteria.TransferUniversityId)
                                                         && (criteria.CourseId == 0
        ...
        [PermissionAuthorize("ExternalCourseEquivalent", PolicyGenerator.Write)]
        ...
        {
            ViewBag.Universities = _selectListProvider.GetTransferUniversities();
            ViewBag.ExternalCourses = _selectListProvider.GetExternalCourses();
